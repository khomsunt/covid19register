...
if ($_GET['risk_level_id']>=0 && $_SESSION['office_type']=='02'){  
  $sql.=" LEFT JOIN (SELECT *,CONCAT(changwat_code,ampur_code) as ampur_code_full FROM office WHERE office_code='".$_SESSION['office_code']."') o2 ON CONCAT(c.changwat_in_code,c.ampur_in_code)=o2.ampur_code_full where v.ampur_code_full=o2.ampur_code_full and c.evaluate_level=".$_GET['risk_level_id'];
}elseif($_GET['risk_level_id']>=0 ){
  $sql.=" where c.hospcode='".$_SESSION['office_code']."' and c.evaluate_level=".$_GET['risk_level_id'];
}elseif($_SESSION['office_type']=='02'){
  $sql.=" LEFT JOIN (SELECT *,CONCAT(changwat_code,ampur_code) as ampur_code_full FROM office WHERE office_code='".$_SESSION['office_code']."') o2 ON CONCAT(c.changwat_in_code,c.ampur_in_code)=o2.ampur_code_full where v.ampur_code_full=o2.ampur_code_full"; 
}else{
